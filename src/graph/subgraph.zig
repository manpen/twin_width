    ...
    return struct {
        ...
        pub fn fromSlice(graph: *Graph(T), nodes: []T) Self {
            ...
            solver.scratch_bitset.unsetAll();
				...
				pub inline fn addContractionAndLeafReduction(self: *Self, seq: *retraceable_contraction.RetraceableContractionSequence(T), min_contraction: contraction.Contraction(T), contractions_left: *u32) !T {
					var total_tww:T = 0;
        ...
        pub fn solveGreedyTopK(self: *Self, comptime K: u32, comptime P:u32, seq: *retraceable_contraction.RetraceableContractionSequence(T), solver: *solver_resources.SolverResources(T,K,P), find_articulation_points: bool) !T {
            	...
            	try self.reduceLeafesAndPaths(K,P,seq,solver);
							...
							while (contractions_left > 0) {
								...
								total_tww = std.math.max(try self.addContractionAndLeafReduction(seq,min_contraction, &contractions_left),total_tww);
							...
							//		var next_selection = try self.selectBestMove(K,P,first_node,solver,total_tww, &select_move_perf_bfs, &select_move_perf_calc);

							//		if(next_selection.potential.tww < selection.potential.tww or (next_selection.potential.tww == selection.potential.tww and next_selection.potential.cumulative_red_edges <= selection.potential.cumulative_red_edges)) {
