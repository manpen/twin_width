...
pub const CompressedBitmapBucketTag = enum(u8) { bitset, array };
...
pub fn FastCompressedBitmap(comptime T: type, comptime promote_threshold: u32, comptime degrade_threshold: u32) type {
    ...
    return struct {
        ...
        pub fn init(max_size: T) Self {
            return Self{ .storage = StorageRepresentation{ .array = edge_list.ParametrizedSortedArrayList(T).init() }, .max_size = max_size };
        }
        ...
        pub fn fromUnsorted(allocator: std.mem.Allocator, list: *edge_list.ParametrizedUnsortedArrayList(T), max_size: T) !Self {
            if (list.cardinality() >= promote_threshold) {
                ...
                const container = Self{ .storage = StorageRepresentation{ .bitset = bitset }, .max_size = max_size };

                return container;
            } else {
                return Self{ .storage = StorageRepresentation{ .array = list.intoSorted() }, .max_size = max_size };
            }
        }
        ...
        pub inline fn xorIterator(self: *const Self, other: *const Self) FastCompressedBitmapXorIterator {
            ...
            return FastCompressedBitmapXorIterator{ .iterator_first = iterator_first, .iterator_second = iterator_second, .item_first = item_first, .item_second = item_second, .first = false };
        }
