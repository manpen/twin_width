...
pub fn TopKScorer(comptime T: type, comptime K: u32) type {
    ...
    return struct {
        ...
        pub const TopKQueueEntry = struct { node: T, score: i32 };
        ...
        pub inline fn iterator(self: *Self, graph: *Graph(T)) !TopKScorerIterator {
            for (self.node_list[0..self.write_ptr]) |item| {
                ...
                if (self.priority_queue.count() < K) {
                    try self.priority_queue.add(TopKQueueEntry{ .node = @intCast(T, item), .score = item_score });
            ...
            return TopKScorerIterator{ .bounded_arr = &self.sorted_targets, .index = 0 };
        }
